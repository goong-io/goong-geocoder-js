 ...
 * @param {Boolean} [options.usefocalLocation=true] This boolean prioritizes results that are close to the map's center location. Default `true`
...
GoongGeocoder.prototype = {
  ...
  _onChange: function() {
    ...
    if (selected && JSON.stringify(selected) !== this.lastSelected) {
      ...
      if (this.options.flyTo) {
        ...
        var r = this._makeRequest(this.options.origin + "/Place/Detail?placeid=" + selected.place_id + "&api_key=" + this.options.accessToken, "GET");
        r.then(function (res) {
  ...
  _makeRequest: function(url, method) {
    var _self = this;
    ...
    return new Promise(function (resolve, reject) {
      ...
      _self.request.onreadystatechange = function () {
        ...
        if (_self.request.readyState !== 4) return;

        // Process the response
        if (_self.request.status >= 200 && _self.request.status < 300) {
          // If successful
          resolve(_self.request);
        } else {
          // If failed
          reject({
            status: _self.request.status,
            statusText: _self.request.statusText
          });
        }
      ...
      _self.request.open(method || 'GET', url, true);
  ...
  _geocode: function(searchInput) {
    ...
    return new Promise(function(resolve, reject){
      var url = self.options.origin + "/Place/Autocomplete?input=" + searchInput + "&limit=" + self.options.limit + "&api_key=" + self.options.accessToken;
      if (self.options.usefocalLocation && self._map) {
        var center = self._map.getCenter();
        url += "&location=" + center.lat + "," + center.lng + "&radius=3000";        
      }
      var r = self._makeRequest(url, "GET");
      r.then(function (response) {
        ...
        if (res.predictions.length) {
          ...
          self._eventEmitter.emit('results', res);
          self._typeahead.update(res.predictions);
        } else {
          self._clearEl.style.display = 'none';
          self._typeahead.selected = null;
        ...
        self._typeahead.selected = null;
        self._renderError();
        self._eventEmitter.emit('results', { predictions: [] });
        self._eventEmitter.emit('error', { error: error });
        reject(error);
      });
    });
  },
